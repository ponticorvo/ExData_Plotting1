...
#Plot a histogram of the Global Active power data
png("plot1.png", width = 480, height = 480)
hist(df.sub$Global_active_power, main = "Global Active power", col = "red", xlab = "Global Active Power (kilowatts)", )
dev.off()

png("plot2.png", width = 480, height = 480)
plot(df.sub$Time, df.sub$Global_active_power, xlab = "", ylab = "Global Active Power (kilowatts)", type = "l")
dev.off()

png("plot3.png", width = 480, height = 480)
plot(x=df.sub$Time,y=df.sub$Sub_metering_1,type="l", xlab="",ylab="Energy Sub Meetering")
